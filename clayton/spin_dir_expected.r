...
train_matrix <- xgb.DMatrix(data = as.matrix(train_data %>% select(-spin_axis)), 
                            label = train_data$spin_axis)
test_matrix <- xgb.DMatrix(data = as.matrix(test_data %>% select(-spin_axis)), 
                           label = test_data$spin_axis)
...
# Make predictions
predictions <- predict(xgb_model, as.matrix(test_data %>% select(-spin_axis)))
test_metadata <- bind_cols(test_metadata, data.frame(pfx_z_predictions = predictions))

# Calculate the RMSE
rmse <- sqrt(mean((predictions - test_data$spin_axis)^2))
