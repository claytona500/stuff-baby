...
# Calculate the RMSE
rmse <- sqrt(mean((predictions - test_data$pfx_x_cor)^2))
...
test_data_with_predictions <- bind_cols(test_metadata, test_data, data.frame(pfx_x_cor_predicted = test_predictions))

big_data <- test_data_with_predictions %>%
    mutate(
        pfx_x_cor_diff = pfx_x_cor - pfx_x_cor_predicted,
        pfx_x_cor_diff_abs = abs(pfx_x_cor)
    ) %>%
    select(pitch_id_raw, pfx_x_cor, pfx_x_cor_predicted, pfx_x_cor_diff, pfx_x_cor_diff_abs)
...
filename <- "2022&3_pfx_x_predictions.csv"
