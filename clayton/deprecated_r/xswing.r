...
savant_clean  <- savant %>%
    ...
    vx0, vy0, vz0, ay, ax, az, spin_axis, release_pos_y, release_speed, release_pos_z, swing,
...
train_matrix <- xgb.DMatrix(data = as.matrix(train_data %>% select(-swing)), 
                            label = train_data$swing)
test_matrix <- xgb.DMatrix(data = as.matrix(test_data %>% select(-swing)), 
...
# Make predictions
predictions <- predict(xgb_model, as.matrix(test_data %>% select(-swing)))
test_metadata <- bind_cols(test_metadata, data.frame(xswing = predictions))

# Calculate the RMSE
rmse <- sqrt(mean((predictions - test_data$swing)^2))

# Create a data frame with the predictions and the actual values
predictions_df <- data.frame(predictions, test_data$swing)

# Rename the columns
colnames(predictions_df) <- c("predictions", "actual")

# Make predictions
test_predictions <- predict(xgb_model, as.matrix(test_data %>% select(-swing)))

# Combine the predictions with the metadata
test_data_with_predictions <- bind_cols(test_metadata, test_data, data.frame(swing_predicted = test_predictions))
...
filename <- "2022&3_swing_predictions.csv"
