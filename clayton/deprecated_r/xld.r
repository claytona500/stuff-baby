...
savant_clean  <- savant %>%
    ...
    vx0, vy0, vz0, ay, ax, az, spin_axis, release_pos_y, release_speed, release_pos_z, ld,
...
train_matrix <- xgb.DMatrix(data = as.matrix(train_data %>% select(-ld)), 
                            label = train_data$ld)
test_matrix <- xgb.DMatrix(data = as.matrix(test_data %>% select(-ld)), 
                           label = test_data$ld)
...
# Make predictions
predictions <- predict(xgb_model, as.matrix(test_data %>% select(-ld)))
test_metadata <- bind_cols(test_metadata, data.frame(xld = predictions))


# Calculate the RMSE
rmse <- sqrt(mean((predictions - test_data$ld)^2))

# Create a data frame with the predictions and the actual values
predictions_df <- data.frame(predictions, test_data$ld)

# Rename the columns
colnames(predictions_df) <- c("predictions", "actual")

test_predictions <- predict(xgb_model, as.matrix(test_data %>% select(-ld)))

# Combine the predictions with the metadata
test_data_with_predictions <- bind_cols(test_metadata, test_data, data.frame(ld_predicted = test_predictions))
...
filename <- "2022&3_ld_predictions.csv"
